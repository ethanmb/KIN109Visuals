...
function updateCorrelationLine() {
    if (data.length < 2) {
        d3.select("#equation").html(""); // Clear equation if not enough data
        return;
    }
    ...
    const slope = d3.sum(data, d => (d.x - meanX) * (d.y - meanY)) /
                  d3.sum(data, d => (d.x - meanX) ** 2);
    const intercept = meanY - slope * meanX;

    // Ensure the line starts and ends within the x-axis domain
    const xMin = xScale.domain()[0];
    const xMax = xScale.domain()[1];

    // Compute y-coordinates for the regression line in the data space
    const yMinData = slope * xMin + intercept; // y-value at xMin
    const yMaxData = slope * xMax + intercept; // y-value at xMax

    // Adjust for the scaled y-axis range
    const yMinVisual = yScale(yMinData); // Map yMin to pixel space
    const yMaxVisual = yScale(yMaxData); // Map yMax to pixel space

    regressionLine
        .attr("x1", xScale(xMin)) // xMin mapped to pixel space
        .attr("y1", yMinVisual)  // yMin mapped to pixel space
        .attr("x2", xScale(xMax)) // xMax mapped to pixel space
        .attr("y2", yMaxVisual);  // yMax mapped to pixel space

    // Debugging: Log key values for verification
    console.log(`Line equation: y = ${slope.toFixed(2)}x + ${intercept.toFixed(2)}`);
    console.log(`Data intercept (at x=0): ${intercept}`);
    console.log(`Visual intercept (mapped): ${yScale(intercept)}`);

    // Update the equation dynamically below the chart
    const slopeFormatted = slope.toFixed(2);
    const interceptFormatted = intercept.toFixed(2);
    d3.select("#equation").html(`Equation of the Line: <strong>y = ${slopeFormatted}x + ${interceptFormatted}</strong>`);
}
